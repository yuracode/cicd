...
4. **境界値テスト** を純粋関数に対して書く：0・1・最大値・負数・極端に大きい値・空配列・空文字。**普段書かない入力** で本当に壊れないか確認

```js
// 例：formatTime(seconds)
expect(formatTime(0)).toBe('00:00')
expect(formatTime(-1)).toBe('00:00')       // 想定外入力の扱い
expect(formatTime(99999)).toBe('27:46:39')  // 極端な値
```

5. **「仕様書としてのテスト」** 視点で見直す：自分のテストを初めて読む人（＝引継ぎ先の開発者）が、**実装を読まなくてもアプリの仕様が分かる** 状態か。`it('...')` の文言が全て日本語で意味を成すかチェック

6. **テスト追加中に見つけたバグを Issue 化** してから PR で直す流れを実行：
   - Issue 作成（タイトル：「[BUG] タイマーが0になる前にリセットされる」等）
   - `fix/issue-3` ブランチを切る
   - PR 本文に `Closes #3`
   - マージで Issue も自動close

> **狙い：** 「気づいた → すぐ直す」ではなく、**記録に残す** 習慣。実務では数週間後に同じバグが再発した時の照会材料になる。

### チャレンジ課題（挑戦）

7. **テストしにくいコードをリファクタリング** する：
   - `useEffect` の中で直接 `fetch` を呼んでいる → API呼び出しを別関数に出す
   - コンポーネント内に巨大な計算ロジックがある → `utils.js` に出す
   - 副作用と描画が混ざっている → 分離する

→ **「テストが書けない」＝「設計を見直すタイミング」**。書きにくさをシグナルとして使う。

8. **E2E テストに挑戦**（時間に余裕があれば）：Playwright か Cypress を調べて、1本だけ「アプリを起動してタイマーを開始して停止する」流れをブラウザ自動操作で書く。単体テストとの違いを体感
