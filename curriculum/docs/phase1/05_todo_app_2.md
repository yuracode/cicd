...
2. **「再読み込み」ボタン** を追加し、押したらAPIから再取得するようにする（state に `reloadKey` を持たせて useEffect の依存配列に入れる）

### 応用課題（推奨）

3. **ローカルTODOとAPI TODOを切り替えるタブ** を作る。上部に「自分のTODO」「サンプル（API）」の2タブ。切り替えても、タブごとの状態は保たれるように設計する（APIは毎回再取得で可）
4. `localStorage` への保存を **保存タイミングで減らす** 。現状だと「TODOが変わるたびに全件JSON.stringify」になっている。1秒に1回にまとめる仕組みを自分で考えて実装する（ヒント：`setTimeout` + `clearTimeout` の組み合わせ。React界隈では「デバウンス」と呼ばれる）

> **狙い：** 「動く」→「効率よく動く」へステップアップ。素直に書くと無駄な処理が多いことに気づく練習。

### チャレンジ課題（挑戦）

5. `localStorage` の容量制限（ブラウザで大体5MB）を調べて、**超えた時の挙動** を実験する。巨大な文字列（`'a'.repeat(10_000_000)` 等）を入れてみると `QuotaExceededError` が出る。try-catch でこれをハンドリングし、画面に「容量が足りません」と表示する
6. fetch が **3秒でタイムアウト** するようにする。3秒以内に応答がなければエラー扱い。ヒント：`AbortController` を使う。MDNで調べて自力で組み立てる（実装方針は1つではないので、自分のやり方でOK）

```js
// ヒント：AbortController の使い方
