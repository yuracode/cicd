...
2. プロジェクトを `git init` しておく（リモートへのpushは次々回）
...
3. `App.jsx` の中に、自己紹介用のHTMLを **自分で考えて** 書く（名前・出身・好きな言語・3年後の目標 など）。インラインstyleで文字色・背景色・余白を調整して、見やすい形に仕上げる
4. 開発サーバを起動したまま `App.jsx` をわざと壊してみる（`<h1>` の閉じタグを消す、等）。画面やターミナルにどんなエラーが出るかを **自分の言葉でメモ** に残す → 直して復帰する

> **狙い：** 壊す→読む→直す のサイクルは今後ずっと続く基本動作。最初の回から慣れておく。

### チャレンジ課題（挑戦）

5. `npm run build` を実行し、`dist/` ディレクトリに何が入っているかを観察する。`dist/index.html` をブラウザで **直接ダブルクリックで開く** とアプリが真っ白になる。なぜか理由を調べて、1〜2行でまとめてみる（ヒント：`file://` プロトコル、JavaScriptモジュールの読み込み、絶対パス）
6. `package.json` の `scripts` に新しいコマンド `"greet": "echo Hello from npm"` を追加し、`npm run greet` で動くことを確認する。scripts の仕組みを **自分で** 1つ作れたかがゴール
